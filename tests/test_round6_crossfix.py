...
def _json_helpers() -> str:
    """static/app.js の postJson / getJson の部分だけ取り出す。"""
    js = (STATIC / "app.js").read_text(encoding="utf-8")
    return js[js.index("async function postJson("):js.index("// ---- ジョブの進捗")]
...
class N {
  constructor(tag) {
    ...
    this._text = ""; this.disabled = false; this.className = ""; this.value = "";
  }
  ...
  querySelector(sel) { return (this.one || {})[sel] || null; }
  querySelectorAll(sel) { return (this.many || {})[sel] || []; }
}
globalThis.Node = N;
...
globalThis.document = {
  ...
  querySelector: (sel) => globalThis.roots[sel] || null,
  querySelectorAll: (sel) => globalThis.rootsAll[sel] || [],
  addEventListener() {},
};
globalThis.rootsAll = {};
globalThis.toasts = [];
...
globalThis.fetch = async () => ({{
  ok: false, status: 400,
  json: async () => ({{ error: "キーの列を1つ選んでください", errors: ["キーの列を1つ選んでください", "日付の列がありません", "見出しが重なっています"] }}),
}});
const errors = new N("ul");
const editor = new N("div");
editor.dataset.saveUrl = "/tables/imports/1/columns";
editor.dataset.headerRowsCount = "1";
editor.one = {{ "[data-editor-errors]": errors }};
editor.many = {{ "[data-setting]": [], "tr[data-col]": [] }};
const saveButton = new N("button");
globalThis.roots["[data-columns-editor]"] = editor;
globalThis.rootsAll["[data-editor-save]"] = [saveButton];
globalThis.window.App = {{ toast: (m) => globalThis.toasts.push(m), postJson, getJson }};
{_tables_js()}
(async () => {{
  await Promise.all(saveButton.fire("click", {{ currentTarget: saveButton }}));
...
{_json_helpers()}
globalThis.fetch = async () => ({{
  ok: false, status: 409, json: async () => ({{ error: "ほかの処理の最中です" }}),
}});
const errors = new N("ul");
const editor = new N("div");
editor.dataset.saveUrl = "/tables/imports/1/columns";
...
const out = new N("p");
const aiPage = new N("div");
aiPage.dataset.estimateUrl = "/tables/imports/1/ai/estimate";
aiPage.one = {{ "[data-estimate-run]": button, "[data-estimate]": out }};
aiPage.many = {{}};
globalThis.roots["[data-ai-page]"] = aiPage;
globalThis.window.App = {{ toast: (m) => globalThis.toasts.push(m), postJson, getJson }};
{_tables_js()}
(async () => {{
  const running = Promise.all(button.fire("click", {{ currentTarget: button }}));
