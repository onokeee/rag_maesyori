...
| 帳票登録で置く Excel（ファイルは残さない。シートの中身だけ覚える） | この帳票のExcel | 見本、見本ファイル、サンプルExcel |
| 帳票の種類が持っている、登録したときのシートの中身（`pattern_books`） | 覚えたシート | スナップショット、キャッシュ |
...
**Excel のファイルは保存しない**（利用者の指示 2026-09-21。3.3「帳票登録は Excel のファイルを1つも残さない」）。その代わり、
登録したときの**シートの中身**（シート名・非表示・セルの番地と文字・型のある値（日時・数値・真偽）・結合・太字・塗りの有る無し・
数式の結果が無いセル・画像の場所・1904年基準）を `forms.WorkbookInfo.to_json` の JSON にして、帳票の種類と一緒に `pattern_books`
に覚える（利用者の指示 2026-09-22「再度シートを置かなくても、登録したときにシートのセル番地と文字情報を記憶しておけばだせるはず」）。
**セルの色は覚えない**（同日「セル色の情報は不要」）。読み取りが使うのは「塗ってあるか」と「2つのセルが同じ塗りか」だけ
（見出し・列見出し・ラベル欄の終わりの判定）なので、`Cell.fill` はブック内で同じ色なら同じ番号、という**組の番号**（1, 2, 3… 見つけた順。
塗りなしは 0。`forms.fill_group`）だけにし、色の値（RGB・テーマ色）は開いた要求の中で捨てて JSON にも書かない。列幅・罫線・数式・
画像の中身も書かない。JSON は1枚の帳票で数KB〜数十KB（見本150件で 3.8KB〜24KB、中央値 10KB）。

- 覚える入口は2つ。`POST /form-types/new`（登録）は JSON を作って大きさを確かめてから種類を作り、`POST /form-types/<id>/panel` に
  ファイルを置いたとき（「別のExcelに替える」）は sha256 かファイル名が違えば入れ替える。種類1つにつき最後に置いた Excel の分を
  **1つ**持ち、種類を消せば一緒に消える（`ON DELETE CASCADE`）。セルのクリックに添えたファイルでは入れ替えない。
- JSON の上限は `views.SNAPSHOT_MAX_BYTES = 4MB`。超えるブック（何万セルもある一覧表のようなもの）は帳票ではないので、
  種類を作らずに断る（減らし方を画面に書く）。置き替えでも前の分を守る。
- `views._request_book(pattern_id)` は **ブラウザが送ってきたファイル → さっき読んだブック（`book_hash`＝sha256。
  `core.workbook_cache`）→ 覚えたシート** の順に探す。だから開き直した種類（`GET /form-types/<id>/panel`）は Excel を置かずに
  登録のときと同じ画面になり、クリック・削除・見出しの手直し・読み取りテスト・使用開始がそのまま動く。覚えたシートから戻した
  `WorkbookInfo` も同じ置き場に短い間だけ置く（クリックのたびに JSON を読み直さない）。
- ブラウザ（`static/app.js` の帳票登録の部分）は選ばれた `File` を `bookFile` に持ち続け、`withBook()` がセルのクリック・項目の削除・
  見出しの手直し・使用開始のたびに `book` という部品で一緒に送る。無ければ `book_hash`、それも無ければ何も足さない
  （サーバーが覚えたシートを使う）。別の種類を開いたら持ち越さない。
...
- `POST /form-types/<id>/panel` が「別の Excel に替える」入口。**種類は増えない**。書き方の違う版に**置き替えて**同じ欄を
  クリックすると、その項目の探す見出しに足される（前の「見本を何枚も預かる」の代わり）。覚えたシートもその Excel の分に入れ替わる。
- 受け取る大きさの上限は `views.form_types.BOOK_MAX_BYTES = 50MB`（`MAX_CONTENT_LENGTH` とは別。保存せずメモリで読むため）。
- 覚える前（`_m14_pattern_books` より前）に登録した種類だけ、覚えたシートが無い。その種類は開き直すと左の欄が置き場
  （`drop-book-edit`）になり、同じ帳票を一度置くとシートを覚えて次からは置かずに開ける。壊れた JSON も同じ扱い（画面は落ちない）。
- 画面の文言は `views.SNAPSHOT_NOTE` の1つ：「Excel のファイルは保存しません。登録したときのシートの中身（セルの番地と文字）だけを
  覚えておき、あとから開いたときにその画面を出します。セルの色は覚えません（塗りの有る無しだけを、見出しの判定のために覚えます）。」
  「サーバーに残しません」「何も残さない」とは言わない（シートの中身は覚えているため）。

1. **この帳票のExcel**：帳票の Excel をドロップ（1ファイル。ファイルは残らず、シートの中身を覚える）。
...
4. **読み取りテスト**：その場で読み取り結果を出す（開き直したときは覚えたシートで出す。覚える前に登録した種類で Excel も
   置いていないときだけ「できません。設定は残っています」と断る）。
5. **使用開始**：何も消さない（もともと Excel のファイルを預かっていない）。押したあとも画面はそのまま続けて使える。

保存した種類を開き直すと（`GET /form-types/<id>/panel`）、登録のときと同じ左右の画面になる。左には登録したときに覚えた
シートがそのまま出て（「登録したときに覚えたシートです（ファイル名、置いた日時）。…セルの色は覚えません」の1行を添える）、
同じようにセルをクリックして直せる。Excel を置き直す必要はない（利用者の指示 2026-09-22）。見出しの横の「別のExcelに替える」で
書き方の違う同じ帳票に替えると、覚えたシートもそれに入れ替わる。
覚える前に登録した種類（覚えたシートが無い）だけ、左の欄が置き場（`ui.file_drop`、id は `drop-book-edit` で「新しく登録する」の
置き場と分ける）になり、同じ帳票を置くと `POST /form-types/<id>/panel` でシートが出て覚える。置くまでは項目の一覧と見出しの
手直しだけができ、値の欄は「—（左にExcelを置くと、この設定で読んだ値が出ます。シートを覚える前に登録した種類です）」。
...
帳票の種類が持つのは `patterns` / `pattern_sheets` / `pattern_fields` の設定と、登録したときの**覚えたシート**（2.5）:
```
pattern_books(pattern_id PK REFERENCES patterns(id) ON DELETE CASCADE,   -- 種類1つに1つ（マイグレーション _m14_pattern_books。user_version 14）
              file_name, file_hash,   -- 置いた Excel の名前と sha256（表示用・ブラウザの book_hash と同じもの。ファイルそのものは無い）
              book_json TEXT,         -- forms.WorkbookInfo.to_json（セルの番地と文字・結合・太字・塗りの有る無しと組の番号。色は無い）
              saved_at)
```
...
- **途中のものは捨てる（2026-09-20）**：作業中の一覧もダウンロード待ちの一覧も持たない（0.1）ので、
  ...
  - **帳票登録は Excel のファイルを預からない**（2026-09-21 の利用者の指示）ので、捨てる対象がそもそも無い。
    置かれた Excel は要求の中で読み取ってファイルは捨て、`uploads/` にも DB にも残さない。残るのは読み取ったシートの
    中身の JSON（`pattern_books`。2.5）だけで、これは種類の設定の一部（下の「帳票登録は Excel のファイルを1つも残さない」）。
...
- **帳票登録は Excel のファイルを1つも残さない**（利用者の指示 2026-09-21「帳票登録で、見本のExcelは置かずに、設定だけ保持する
  ...
  **前の版が残した `uploads/samples` フォルダだけ**を片付ける。
  残さないのはファイル（bytes）。**シートの中身の JSON（`pattern_books`。2.5）は設定として残る**（利用者の指示 2026-09-22
  「再度シートを置かなくても、登録したときにシートのセル番地と文字情報を記憶しておけばだせるはず」）。`core.SETTINGS_TABLES` に
  入れて `purge_session` / `sweep_stale` / `purge_all_pending` の対象にしない。消えるのは種類を消したときだけ（`ON DELETE CASCADE`）。
  JSON に Excel のバイト（zip）は入らず、セルの色も入らない（同日「セル色の情報は不要」。塗りの有る無しと、同じ塗りかを比べる
  ための組の番号だけ）。
  帳票の種類が持つのは設定：シート名・見出しのセル・値のセル・読み取る向き・項目名と、登録したときの覚えたシート。
...
- 帳票登録で置いた Excel のファイルは**そもそも保存しない**（2026-09-21。読み取った結果だけを `core/workbook_cache.py` が
  メモリに短い間だけ覚える）。［使用開始］は何も消さない。登録したときのシートの中身（セルの番地と文字。色は無い）は
  `pattern_books` が種類と一緒に持つ（2026-09-22。2.5）ので、あとから項目を直すときに Excel を置き直す必要はない。
