...
Excel/CSV の**一覧表（1行＝1件）**を読み取り、LightRAG に手作業で投入しやすい Markdown(.md) を作ってダウンロードする Flask アプリです。社内LANのサーバーで動かし、数人が自分のPCのブラウザから同時に使います。ログインはありません。LightRAG への送信はしません（ダウンロードまで）。

取り込んだデータはサーバーに残しません。Markdown をダウンロードした時点で、元のファイル・読み取った内容・作ったファイル・DBの行を消します。残るのはブラウザごとの AI接続の設定だけです。
...
| 画面 | アドレス | できること |
|---|---|---|
| 表の取り込み | `/tables`（`/` はここへ） | Excel/CSV の一覧表を読み取って Markdown（zip）にする |
| 解説 | `/guide` | 何を手がかりにどう決めているか（目次つき）。しきい値はコードの定数と同じ値 |
...
帳票（1ファイル＝1件）の取り込みと帳票登録は 2026-09-26 に削除しました（セル単位のルールでは実帳票に合わなかったため）。必要になったときは `a207ffd` より前のコミットに残っています。
