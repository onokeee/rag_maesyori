"""大規模サンプルデータ（一覧表 T1〜T5 / 帳票 F1〜F5）をまとめて生成する。

    python scripts/generate_samples.py                  # samples/ に全10種を出力
    python scripts/generate_samples.py --only T1,F3     # 指定したものだけ
    python scripts/generate_samples.py --out D:/tmp/s   # 出力先を変える
    python -m scripts.generate_samples                  # モジュールとしても実行できる

各ジェネレータは scripts/samples/<module>.py の generate(output_root) で、固定シードのため
何度実行しても同じ内容になる。最後に生成結果の一覧（ファイル数・行数・サイズ・秒数）を表示し、
出力先に README.md（データセットの目録）を書き出す。
"""
...
# 「python scripts/generate_samples.py」で直接実行した場合もパッケージを import できるようにする
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        pass
    parser = argparse.ArgumentParser(description="サンプルデータ（T1〜T5 / F1〜F5）を生成する")
    parser.add_argument("--only", help="生成する対象をカンマ区切りで指定（例: T1,F3）")
