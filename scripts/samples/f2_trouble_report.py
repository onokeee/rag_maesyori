...
- xlsx は様式の版ごとのサブフォルダ（Rev3_2025年3月まで／Rev5_2025年4月改訂）に分けて出力する。
...
# Windows のフォルダ名に使えない文字（. : / \ * ? " < > |）も使わない（`Rev.5` → `Rev5`）。
# 旧様式 Rev.3 は制定・改訂の時期が帳票に載っていないので、代わりに使われていた時期（いつまで）を書く。
# 2版とも 2025年なので、月まで入れないと入れ替わりが分からない。
VERSION_DIR = {
    OLD: "Rev3_2025年3月まで",
    NEW: "Rev5_2025年4月改訂",
}
...
def _readme(reports: list[Report]) -> str:
    ...
    lines = [
        ...
        "",
        "フォルダ名は「版の名前＋いつから」です（旧様式 Rev.3 は制定・改訂の時期が帳票に載っていないため、使われていた時期を書いています）。",
        "",
