"""Excel/CSV の一覧表（1行＝1件）の読み取りと Markdown の組み立て。

- 読み取り方の判定（文字コード・区切り・シート）→ 表の範囲（見出し行・データ行）→ 列の対応づけ →
  値の正規化 → Markdown（記録ファイル）の順で、1本の流れとして通す。
- 決まりは2つだけ: 読み取った文字は1つも削らない。同じ入力からは何度でも同じ Markdown。
- 1ファイルの中は「# 元 tables/*.py」の見出しで、前の構成のモジュールごとに区切ってある。
"""
...
####################################################################################################
# 共通の文字の処理（Excel/CSV のセルの値をそろえる・単位・ラベルの正規化）
####################################################################################################
...
def ambiguous_unit_hint(field_name: str, display_name: str = "") -> str:
    ...
    下の一覧に当てはまる列だけを要確認にする。列の名前（見出し）と標準キーのどちらでも照合する。
    """
    labels = [x for x in (normalize_label(display_name), normalize_label(field_name)) if x]
...
def _labels_down_first_column(head: list[SourceRow], rows_h: list[int], data_start: int, end: int) -> bool:
    """見出し行が2列以下で、下の行の先頭列に項目名（辞書の語）が並ぶなら「ラベル: 値」の並び。"""
...
# 列の見出しから標準キー・型・役割を引く辞書。
#
# キー名（equipment_id・downtime・symptom など）は内部の名前で、画面や Markdown には出ない
# （画面と md に出るのは、その表の見出しの文字そのまま）。
# ====================================================================================================
...
_CODE_MODULES = ("extract.py", "app.py")
...
def _log_stage_errors(stage: LogStageSpec) -> list[str]:
    ...
    from ai import normalize_rules
...
def people_index_for(spec: TableSpec, records: list[dict]):
    ...
    from ai import PeopleIndex, drop_placeholder_names
